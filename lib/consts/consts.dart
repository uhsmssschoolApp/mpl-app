import 'package:flutter/material.dart';

Color darkThemeBg = const Color(0xFF292929);
Color darkThemePrimary = const Color(0xFF303030);
Color darkThemeVariant = const Color(0xFF212121);
Color darkThemeSurface = const Color(0xFF363636);

Color themeSecondary = const Color(0xFFA4D464);

Color lightThemebg = const Color(0xFFF4F2F7);
Color lightThemePrimary = const Color(0xFFFFFFFF);
Color lightThemeVariant = const Color(0xFFF9F9F9);
Color lightThemeOnSurface = const Color(0xFF27242C);

const double kSidePadding = 30;
const double kTopPadding = 70;